...
- 自动采集章节写入新增 SQLite 本地缓冲：新书仍立即写入 MySQL 获取 articleid，章节先进入本地缓存，换书、定时、停止或手动提交时再写入 Jieqi。
- 自动采集任务 UI 新增数据库写入面板，可查看待提交章节、启用状态、提交间隔、批量大小，并提供提交、重试和打开缓存目录。
- 手工追加和顺序插章在插入后立即提交缓存，保持章节列表刷新和 HTML 生成行为与旧版本一致。
- 本阶段 flush 复用既有 Jieqi 插章语义，先降低采集中途目标库压力；后续继续把 flush 内部压缩为单连接单事务批量写入。
